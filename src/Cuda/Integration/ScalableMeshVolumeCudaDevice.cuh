...
__device__ inline int ScalableMeshVolumeCudaDevice::LinearizeNeighborOffset(
        const Vector3i &dXsv) {
...
__device__ inline Vector3i
ScalableMeshVolumeCudaDevice::BoundaryVoxelInNeighbor(const Vector3i &Xlocal,
                                                      const Vector3i &dXsv) {
...
__device__ void ScalableMeshVolumeCudaDevice::AllocateVertex(
        const Vector3i &Xlocal,
        int subvolume_idx,
...
__device__ void ScalableMeshVolumeCudaDevice::AllocateVertexOnBoundary(
        const Vector3i &Xlocal,
        int subvolume_idx,
    ...
    for (size_t edge = 0; edge < 12; ++edge) {
        if (edges & (1 << edge)) {
            ...
            vertex_indices(Xlocal_edge_holder_in_neighbor,
                           neighbor_subvolume_idx)(edge_shift[edge][3]) =
                    VERTEX_TO_ALLOCATE;
        }
    }
}

__device__ void ScalableMeshVolumeCudaDevice::ExtractVertex(
        const Vector3i &Xlocal,
        int subvolume_idx,
        const Vector3i &Xsv,
    ...
    Vector3f gradient_0 = (mesh_.type_ & VertexWithNormal)
                                  ? subvolume->gradient(Xlocal)
                                  : Vector3f::Zeros();

    Vector3b color_0 = (mesh_.type_ & VertexWithColor)
                               ? subvolume->color(Xlocal)
                               : Vector3b::Zeros();
        ...
        if (voxel_vertex_indices(axis) == VERTEX_TO_ALLOCATE) {
            ...
            if (mesh_.type_ & VertexWithNormal) {
                mesh_.vertex_normals_[voxel_vertex_indices(axis)] =
                        tsdf_volume.transform_volume_to_world_
                                .Rotate((1 - mu) * gradient_0 +
...
__device__ void ScalableMeshVolumeCudaDevice::ExtractVertexOnBoundary(
        const Vector3i &Xlocal,
        int subvolume_idx,
        const Vector3i &Xsv,
        ScalableTSDFVolumeCudaDevice &tsdf_volume,
...
#pragma unroll 1
    for (size_t axis = 0; axis < 3; ++axis) {
        if (voxel_vertex_indices(axis) == VERTEX_TO_ALLOCATE) {
            ...
            Vector3i Xneighbor_axis = Xlocal_axis - int(N_) * dXsv_axis;
            float tsdf_axis =
                    cached_subvolumes[neighbor_idx]->tsdf(Xneighbor_axis);
            Vector3b color_axis =
                    cached_subvolumes[neighbor_idx]->color(Xneighbor_axis);
...
__device__ void ScalableMeshVolumeCudaDevice::ExtractTriangle(
...
__device__ void ScalableMeshVolumeCudaDevice::ExtractTriangleOnBoundary(
        const Vector3i &Xlocal,
        int subvolume_idx,
        int *cached_subvolume_indices) {
    const uchar table_index = table_indices(Xlocal, subvolume_idx);
