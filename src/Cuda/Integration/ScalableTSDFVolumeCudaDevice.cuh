...
__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::world_to_voxelf(
        const Vector3f &Xw) {
    return volume_to_voxelf(transform_world_to_volume_ * Xw);
}

__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::voxelf_to_world(
        const Vector3f &X) {
    return transform_volume_to_world_ * voxelf_to_volume(X);
}

__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::voxelf_to_volume(
        const Vector3f &X) {
...
__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::volume_to_voxelf(
        const Vector3f &Xv) {
...
__device__ inline Vector3i ScalableTSDFVolumeCudaDevice::voxel_locate_subvolume(
        const Vector3i &X) {
...
__device__ inline Vector3i
ScalableTSDFVolumeCudaDevice::voxelf_locate_subvolume(const Vector3f &X) {
    return Vector3i(int(floor(X(0) / N_)), int(floor(X(1) / N_)),
                    int(floor(X(2) / N_)));
}

__device__ inline Vector3i ScalableTSDFVolumeCudaDevice::voxel_global_to_local(
        const Vector3i &X, const Vector3i &Xsv) {
    return Vector3i(X(0) - Xsv(0) * N_, X(1) - Xsv(1) * N_, X(2) - Xsv(2) * N_);
}

__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::voxelf_global_to_local(
        const Vector3f &X, const Vector3i &Xsv) {
    return Vector3f(X(0) - Xsv(0) * N_, X(1) - Xsv(1) * N_, X(2) - Xsv(2) * N_);
}

__device__ inline Vector3i ScalableTSDFVolumeCudaDevice::voxel_local_to_global(
        const Vector3i &Xlocal, const Vector3i &Xsv) {
    return Vector3i(Xlocal(0) + Xsv(0) * N_, Xlocal(1) + Xsv(1) * N_,
                    Xlocal(2) + Xsv(2) * N_);
}

__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::voxelf_local_to_global(
        const Vector3f &Xlocal, const Vector3i &Xsv) {
    return Vector3f(Xlocal(0) + Xsv(0) * N_, Xlocal(1) + Xsv(1) * N_,
                    Xlocal(2) + Xsv(2) * N_);
}

/** Query **/

__device__ UniformTSDFVolumeCudaDevice *
ScalableTSDFVolumeCudaDevice::QuerySubvolume(const Vector3i &Xsv) {
    return hash_table_[Xsv];
}

/** Unoptimized access and interpolation **/

__device__ float &ScalableTSDFVolumeCudaDevice::tsdf(const Vector3i &X) {
...
__device__ uchar &ScalableTSDFVolumeCudaDevice::weight(const Vector3i &X) {
    ...
    return subvolume == nullptr
                   ? weight_dummy_
                   : subvolume->weight(voxel_global_to_local(X, Xsv));
}

__device__ Vector3b &ScalableTSDFVolumeCudaDevice::color(const Vector3i &X) {
    Vector3i Xsv = voxel_locate_subvolume(X);
    UniformTSDFVolumeCudaDevice *subvolume = QuerySubvolume(Xsv);

    return subvolume == nullptr
                   ? color_dummy_
                   : subvolume->color(voxel_global_to_local(X, Xsv));
}

__device__ float ScalableTSDFVolumeCudaDevice::TSDFAt(const Vector3f &X) {
    Vector3i Xi = X.template cast<int>();
    Vector3f r = Vector3f(X(0) - Xi(0), X(1) - Xi(1), X(2) - Xi(2));

    return (1 - r(0)) *
                   ((1 - r(1)) * ((1 - r(2)) * tsdf(Xi + Vector3i(0, 0, 0)) +
                                  r(2) * tsdf(Xi + Vector3i(0, 0, 1))) +
                    r(1) * ((1 - r(2)) * tsdf(Xi + Vector3i(0, 1, 0)) +
                            r(2) * tsdf(Xi + Vector3i(0, 1, 1)))) +
           r(0) * ((1 - r(1)) * ((1 - r(2)) * tsdf(Xi + Vector3i(1, 0, 0)) +
...
__device__ uchar ScalableTSDFVolumeCudaDevice::WeightAt(const Vector3f &X) {
    ...
    return uchar(
            (1 - r(0)) *
                    ((1 - r(1)) * ((1 - r(2)) * weight(Xi + Vector3i(0, 0, 0)) +
                                   r(2) * weight(Xi + Vector3i(0, 0, 1))) +
                     r(1) * ((1 - r(2)) * weight(Xi + Vector3i(0, 1, 0)) +
                             r(2) * weight(Xi + Vector3i(0, 1, 1)))) +
            r(0) * ((1 - r(1)) * ((1 - r(2)) * weight(Xi + Vector3i(1, 0, 0)) +
                                  r(2) * weight(Xi + Vector3i(1, 0, 1))) +
                    r(1) * ((1 - r(2)) * weight(Xi + Vector3i(1, 1, 0)) +
                            r(2) * weight(Xi + Vector3i(1, 1, 1)))));
}

__device__ Vector3b ScalableTSDFVolumeCudaDevice::ColorAt(const Vector3f &X) {
    Vector3i Xi = X.template cast<int>();
    Vector3f r = Vector3f(X(0) - Xi(0), X(1) - Xi(1), X(2) - Xi(2));
    ...
    Vector3f colorf =
            (1 - r(0)) *
                    ((1 - r(1)) *
                             ((1 - r(2)) * color(Xi + Vector3i(0, 0, 0))
                                                   .template cast<float>() +
                              r(2) * color(Xi + Vector3i(0, 0, 1))
                                              .template cast<float>()) +
                     r(1) * ((1 - r(2)) * color(Xi + Vector3i(0, 1, 0))
                                                  .template cast<float>() +
                             r(2) * color(Xi + Vector3i(0, 1, 1))
                                             .template cast<float>())) +
            r(0) * ((1 - r(1)) * ((1 - r(2)) * color(Xi + Vector3i(1, 0, 0))
                                                       .template cast<float>() +
                                  r(2) * color(Xi + Vector3i(1, 0, 1))
                                                  .template cast<float>()) +
                    r(1) * ((1 - r(2)) * color(Xi + Vector3i(1, 1, 0))
                                                 .template cast<float>() +
                            r(2) * color(Xi + Vector3i(1, 1, 1))
                                            .template cast<float>()));
...
__device__ Vector3f
ScalableTSDFVolumeCudaDevice::GradientAt(const Vector3f &X) {
...
__device__ inline bool ScalableTSDFVolumeCudaDevice::OnBoundary(
        const Vector3i &Xlocal, bool for_gradient) {
    return for_gradient ? (Xlocal(0) == 0 || Xlocal(1) == 0 || Xlocal(2) == 0 ||
...
__device__ inline bool ScalableTSDFVolumeCudaDevice::OnBoundaryf(
        const Vector3f &Xlocal, bool for_gradient) {
    return for_gradient ? (Xlocal(0) < 1 || Xlocal(1) < 1 || Xlocal(2) < 1 ||
                           Xlocal(0) >= N_ - 2 || Xlocal(1) >= N_ - 2 ||
                           Xlocal(2) >= N_ - 2)
                        : (Xlocal(0) >= N_ - 1 || Xlocal(1) >= N_ - 1 ||
                           Xlocal(2) >= N_ - 1);
...
__device__ inline int ScalableTSDFVolumeCudaDevice::LinearizeNeighborOffset(
        const Vector3i &dXsv) {
...
__device__ inline Vector3i
ScalableTSDFVolumeCudaDevice::BoundaryVoxelInNeighbor(const Vector3i &Xlocal,
                                                      const Vector3i &dXsv) {
    return Vector3i(Xlocal(0) - dXsv(0) * N_, Xlocal(1) - dXsv(1) * N_,
                    Xlocal(2) - dXsv(2) * N_);
}

__device__ inline Vector3f ScalableTSDFVolumeCudaDevice::gradient(
        const Vector3i &Xlocal,
...
#pragma unroll 1
    for (size_t k = 0; k < 3; ++k) {
        ...
        uchar weight1 = (subvolume1 == nullptr)
                                ? 0
                                : subvolume1->weight(
                                          BoundaryVoxelInNeighbor(X1, dXsv1));

        float tsdf0 =
                (subvolume0 == nullptr)
                        ? 0
                        : subvolume0->tsdf(BoundaryVoxelInNeighbor(X0, dXsv0));
        float tsdf1 =
                (subvolume1 == nullptr)
                        ? 0
                        : subvolume1->tsdf(BoundaryVoxelInNeighbor(X1, dXsv1));
...
__device__ float ScalableTSDFVolumeCudaDevice::TSDFOnBoundaryAt(
    ...
    for (size_t k = 0; k < 8; ++k) {
        ...
        float tsdf_k = (subvolume == nullptr)
                               ? 0.0f
                               : subvolume->tsdf(BoundaryVoxelInNeighbor(
                                         Xlocali_k, dXsv_k));
...
__device__ uchar ScalableTSDFVolumeCudaDevice::WeightOnBoundaryAt(
    ...
    for (size_t k = 0; k < 8; ++k) {
        ...
        float weight_k = (subvolume == nullptr)
                                 ? 0.0f
                                 : subvolume->weight(BoundaryVoxelInNeighbor(
                                           Xlocali_k, dXsv_k));
        float weight_interp_k =
                (subvolume == nullptr)
                        ? 0.0f
    ...
    return sum_weight_interp > 0
                   ? uchar(fminf(sum_weight / sum_weight_interp, 255))
                   : uchar(0);
}

__device__ Vector3b ScalableTSDFVolumeCudaDevice::ColorOnBoundaryAt(
        const Vector3f &Xlocal,
        UniformTSDFVolumeCudaDevice **cached_subvolumes) {
    /** X in range: [-1, N_ + 1) **/
#ifdef CUDA_DEBUG_ENABLE_ASSERTION
    ...
    for (size_t k = 0; k < 8; ++k) {
        ...
        Vector3f color_k = (subvolume == nullptr)
                                   ? Vector3f(0)
                                   : subvolume
                                             ->color(BoundaryVoxelInNeighbor(
                                                     Xlocali_k, dXsv_k))
                                             .template cast<float>();
        float weight_interp_k =
                (subvolume == nullptr)
    ...
    return sum_weight_interp > 0
                   ? (sum_color / sum_weight_interp).template cast<uchar>()
                   : Vector3b(0);
}

__device__ Vector3f ScalableTSDFVolumeCudaDevice::GradientOnBoundaryAt(
        const Vector3f &Xlocal,
        UniformTSDFVolumeCudaDevice **cached_subvolumes) {
    Vector3f n = Vector3f::Zeros();
...
__device__ void ScalableTSDFVolumeCudaDevice::ActivateSubvolume(
...
__device__ int ScalableTSDFVolumeCudaDevice::QueryActiveSubvolumeIndex(
        const Vector3i &key) {
    int internal_addr = hash_table_.GetInternalAddrByKey(key);
    return internal_addr == NULLPTR_CUDA
                   ? NULLPTR_CUDA
                   : active_subvolume_indices_[internal_addr];
}

__device__ void ScalableTSDFVolumeCudaDevice::CacheNeighborSubvolumes(
        const Vector3i &Xsv,
        const Vector3i &dXsv,
...
__device__ void ScalableTSDFVolumeCudaDevice::TouchSubvolume(
        const Vector2i &p,
...
__device__ void ScalableTSDFVolumeCudaDevice::Integrate(
        const Vector3i &Xlocal,
        HashEntry<Vector3i> &entry,
    ...
    Vector3f X =
            voxelf_local_to_global(Xlocal.template cast<float>(), entry.key);
    ...
    UniformTSDFVolumeCudaDevice *subvolume =
            hash_table_.GetValuePtrByInternalAddr(entry.internal_addr);
...
__device__ bool ScalableTSDFVolumeCudaDevice::RayCasting(
    ...
    while (t_curr < t_max) {
        ...
        float tsdf_curr =
                is_subvolume_valid ? subvolume->tsdf(Xlocal_t) : tsdf_prev;
        uchar weight_curr =
